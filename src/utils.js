...
export function getOldSalaryIncrease(data) {
  const listApproved = data.filter(item => item.salaryIncreaseStatus === 3);
  let result = null;
  listApproved.forEach(item => {
    if (item.oldSalary > result) {
      return result = item.oldSalary;
    }
  })
  return result;
}